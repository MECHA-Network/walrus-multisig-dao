...
import { growthbook, setAttributes } from '../../shared/experimentation/features';
...
accountsEvents.on('accountsChanged', async () => {
	...
	await Permissions.ensurePermissionAccountsUpdated(
		await Promise.all(
			(await getAllAccounts()).map(async (anAccount: { address: any; }) => ({ address: await anAccount.address })),
		),
	);
});
